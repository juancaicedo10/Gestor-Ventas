...
const Notificaciones: React.FC<Props> = ({ isOpen, onClose }) => {
    ...
    const [selectedFechaInicio, setSelectedFechaInicio] = useState<string | undefined>(undefined);

    const [selectedFechaFin, setSelectedFechaFin] = useState<string | undefined>(undefined);

    const [selectedVentas, setSelectedVentas] = useState<boolean | undefined>(undefined);

    const [selectedAbonos, setSelectedAbonos] = useState<boolean | undefined>(undefined);

    const [selectedGastos, setSelectedGastos] = useState<boolean | undefined>(undefined);
    ...
    }, [selectedSeller, selectedFechaInicio, selectedFechaFin, selectedVentas, selectedAbonos, selectedGastos]);
  ...
  return (
    ...
    >
      <header className="w-full bg-white py-2 shadow-md flex-col">
        ...
        <h2 className="font-bold text-blue-800 p-2 w-full text-center my-auto text-3xl">Movimientos</h2>
        </div>
        ...
        <Select
                    ...
                    value={options.find(option => option.value === selectedSeller) || null}
                    maxMenuHeight={170}
                    menuPlacement="auto"
                    className="w-3/4"
                  />
         <section className="my-2">
            <input type="date" className="border-2 rounded-md p-1 mr-2" 
            onChange={
              (e) => setSelectedFechaInicio(e.target.value)
            }
            value={selectedFechaInicio || ""}
            />
            a
            <input type="date" className="border-2 rounded-md p-1 ml-2" 
         ...
         <section className="w-1/2 py-1 flex justify-between items-center">
            <div>
              ...
              onChange={
                (e) => setSelectedVentas(e.target.checked ? true : undefined)
              }/>
            </div>
            <div>
              ...
              onChange={
                (e) => setSelectedAbonos(e.target.checked ? true : undefined)
              }/>
            </div>
            <div>
              ...
              onChange={
                (e) => setSelectedGastos(e.target.checked ? true : undefined)
              }
              />
            </div>
         </section>
  ...
  <li key={index} className={ `bg-white rounded-2xl w-1/2 p-2 shadow-sm ${index % 2 !== 0 && 'ml-auto'}`}>
    ...
    <>
      se registró una venta por un valor de <span className="text-blue-600 font-semibold">{notificacion.Valor}$</span> para el cliente <span className="text-blue-600 font-semibold">{notificacion.NombreCliente}</span> con un valor seguro de <span className="font-semibold text-blue-600">{notificacion.ValorSeguro}$</span>
    </>
  ) : notificacion.TipoId === 2 ? (
    <>
      se registró un abono por un valor de <span className="font-semibold text-blue-600">
      {notificacion.Valor}$
        </span> con un interes de <span className="font-semibold text-blue-600">
        {notificacion.ValorInteres}$
          </span> y una multa de <span className="font-semibold text-blue-600">
          {notificacion.ValorMulta}$
            </span> para la venta <Link to={`/cuotas/${notificacion.VentaId}/${notificacion.NumeroVenta}`} className="font-semibold text-blue-600 border-b-2 border-blue-600">{notificacion.NumeroVenta}</Link>
    </>
  ) : notificacion.TipoId === 3 && (
    <>
      se registró un gasto por un valor de <span className="font-semibold text-blue-600">
      {notificacion.Valor}$ 
        </span> para <span className="font-semibold text-blue-600">
