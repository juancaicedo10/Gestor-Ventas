...
const NotificacionesLiquidacion: React.FC<Props> = ({ isOpen, onClose, Consecutivo, VendedorId }) => {
    ...
    >
      <header className="flex w-full bg-white shadow-md py-2">
        ...
        <h2 className="text-center text-2xl w-full font-bold overflow-hidden text-blue-800 my-auto">
           Movimientos Liquidacion <br /> <span className="text-blue-600">
           {Consecutivo}
            </span>
        </h2>
      </header>
  ...
  {notificacion.TipoId === 1 ? (
    <>
      se registró una venta por un valor de <span className="text-blue-600 font-semibold">{notificacion.Valor}$</span> para el cliente <span className="text-blue-600 font-semibold">
        ...
        </span> con un valor seguro de {notificacion.ValorSeguro}$
    </>
  ) : notificacion.TipoId === 2 ? (
    <>
      se registró un abono por un valor de <span className="font-semibold text-blue-700">
      {notificacion.Valor}$
        </span>con un interes de <span className="text-blue-600 font-semibold">
        {notificacion.ValorInteres}$
          </span>y una multa de <span className="text-blue-600 font-semibold">
          {notificacion.ValorMulta}$ 
            </span> para la venta <Link to={`/cuotas/${notificacion.VentaId}/${notificacion.NumeroVenta}`}>
