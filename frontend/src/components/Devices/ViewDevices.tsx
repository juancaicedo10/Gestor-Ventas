...
export default function ViewDevices() {
  ...
  const getDevices = async () => {
    setIsLoading(true);
    try {
      const response = await HttpClient.get(`${import.meta.env.VITE_API_URL}/api/dispositivos/all`);
  ...
  const toggleDeviceState = async (id: number, currentState: boolean) => {
    try {
      const url = `${import.meta.env.VITE_API_URL}/api/dispositivos/${id}/${currentState ? "disable" : "enable"}`;
      await HttpClient.put(url);
      await getDevices(); // Actualiza el estado de todos
        toast.success(`Dispositivo ${currentState ? "deshabilitado" : "habilitado"} correctamente`);
    } catch (err) {
  ...
  return (
    <section className="w-full overflow-y-hidden">
      <Sidebar />
      <div className="flex flex-col justify-center ml-[64px]">
                ...
                {devices.map((device) => (
                  <li
                    ...
                    className={`group p-0 rounded-xl border shadow-md bg-white hover:shadow-lg transition-shadow duration-200 relative overflow-hidden ${
                      selectedId === device.Id ? "border-2 border-quaternary" : ""
                    }`}
                  >
                    {/* Cabecera morada de la card */}
                    <div className="flex items-center justify-between p-3 bg-primary text-white">
                      <div className="flex items-center gap-2">
                        <Person className="text-white bg-primary rounded-full p-1" fontSize="medium" />
                        <div>
                          <h2 className="text-lg font-semibold">
                            {device.Rol === "Vendedor" ? device.NombreVendedor : device.NombreAdministrador}
                          </h2>
                          <p className="text-sm text-white/70">{device.Rol}</p>
                        </div>
                      </div>
                      <button
                        onClick={() => toggleDeviceState(device.Id, device.Autorizado)}
                    ...
                    <div className="space-y-2 text-gray-700 text-sm p-4">
                      <div className="flex items-center gap-2">
                        <DevicesOther className="text-quaternary" fontSize="small" />
                        <span><strong>Modelo:</strong> {device.DeviceModel}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Memory className="text-quaternary" fontSize="small" />
                        <span><strong>Sistema:</strong> {device.DeviceOs}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-quaternary font-bold">Estado:</span>
                        <span className={`text-sm font-medium ${device.Autorizado ? "text-green-600" : "text-red-600"}`}>
                          {device.Autorizado ? "Autorizado" : "No autorizado"}
                        </span>
                      </div>
                      <div className="text-xs text-gray-400 pt-2 border-t">
                        ID Dispositivo: <span className="font-semibold">{device.Id}</span>
                      </div>
                    </div>
                  </li>
                ))}
