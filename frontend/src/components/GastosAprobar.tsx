...
function GastosAprobar() {
  ...
  const getgastosAprobar = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get(`https://backend-gestor-ventas.onrender.com/api/gastos/aprobar/${decodeToken()?.user?.Id}/all`, {
        headers: {
  ...
  const handleAprobar = async (gastoId: number) => {
    ...
    try {
      await axios.put(
        `https://backend-gestor-ventas.onrender.com/api/gastos/${gastoId}/aprobar`,
        { aprobado: true },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
