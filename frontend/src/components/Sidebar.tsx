...
export default function Sidebar() {
  ...
  return (
    <aside className={`z-50 h-dvh flex fixed md:mr-9 flex-col bg-blue-400 transition-all duration-500 ease-in-out ${show ? 'w-3/4 sm:w-1/2 md:w-1/3 lg:w-1/4' : 'w-16'}`}>
      ...
      <ul className="text-2xl flex flex-col w-full">
        <li className="flex hover:text-white hover:bg-blue-500 rounded-md m-2 p-2 cursor-pointer justify-start">
          <Link to="/vendedores" className="flex text-white justify-center items-center" onClick={() =>setShow(false)}>
            ...
            <p hidden={!show} className="font-normal">Vendedores</p>
          </Link>
          </li>
        <li className="flex hover:text-white hover:bg-blue-500 rounded-md m-2 p-2 cursor-pointer justify-start">
          <Link to='/clientes' className="flex text-white" onClick={() =>setShow(false)}>
          <PeopleIcon fontSize="large"/>
          <p hidden={!show} className="font-normal">Clientes</p>
          </Link>
        </li>
        <li className="flex hover:text-white hover:bg-blue-500 rounded-md m-2 p-2 cursor-pointer justify-start">
          <Link to='/ventas' className="flex text-white" onClick={() =>setShow(false)}>
        <AttachMoneyIcon fontSize="large"/>
          <p hidden={!show} className="font-normal">Ventas</p>
          </Link>
        </li>
        <li className="flex hover:text-white hover:bg-blue-500 rounded-md m-2 p-2 cursor-pointer justify-start">
          <Link to='/ventas' className="flex text-white" onClick={() =>setShow(false)}>
        <AttachMoneyIcon fontSize="large"/>
          <p hidden={!show} className="font-normal">Panel de Control</p>
          </Link>
        </li>
        ...
        <AdminPanelSettingsIcon fontSize="large"/>
          <p hidden={!show} className="font-normal">Clientes por aprobar</p>
          </Link>
        </li>
        }
        <li className="flex hover:text-white hover:bg-blue-500 rounded-md m-2 p-2 cursor-pointer justify-start">
          <AccountCircleIcon fontSize="large"/>
          <p hidden={!show} className="font-normal">Perfil</p>
        </li>
      </ul>
      <li onClick={() => handleLogout()}>
      <button className="m-2 py-2 px-4 rounded-lg text-white text-2xl font-bold bg-blue-900 absolute bottom-0 mx-auto">
        <LogoutIcon fontSize="large"/>
        <p hidden={!show} className="font-normal">Cerrar Sesion</p>
      </button>
      </li>
    </aside>
  )
