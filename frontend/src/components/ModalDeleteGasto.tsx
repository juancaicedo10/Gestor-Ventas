...
interface ModalProps {
  ...
  onClose: () => void;
  getGastos: () => void;
  gastoId: number; // El id del gasto que se va a eliminar
}

const EliminarGastoModal: React.FC<ModalProps> = ({
  isOpen,
  onClose,
  getGastos,
  gastoId,
  ...
  const handleDelete = () => {
    try {
      axios
        .delete(
          `https://backendgestorventas.azurewebsites.net/api/gastos/${gastoId}`,
          {
            headers: {
      ...
      console.log("Error eliminando gasto: ", error);
    }
  };
