...
const RelacionAdministradorVendedorModal: React.FC<ModalProps> = ({
  ...
  const handleSelectSeller = (seleccionado: number) => {
    ...
    axios
      .get(
        `https://backendgestorventas1.azurewebsites.net/api/administradores/relacion/${seleccionado}`
      )
      .then((res) => {
  ...
  const getAdministradores = async () => {
    try {
      const res = await axios.get(
        "https://backendgestorventas1.azurewebsites.net/api/administradores"
      );
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    axios
      .post(
        "https://backendgestorventas1.azurewebsites.net/api/administradores/relacion",
        {
