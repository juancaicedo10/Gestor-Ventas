...
function AbonosyRetiros() {
  ...
  const getAbonosyRetiros = async (VendedorId: number) => {
    ...
    Promise.all([
      axios.get(`https://backendgestorventas1.azurewebsites.net/api/abonos/${VendedorId}`),
      axios.get(`https://backendgestorventas1.azurewebsites.net/api/retiros/${VendedorId}`),
    ])
